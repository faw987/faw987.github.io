...
### Aggregations
- **Create / Add Aggregation** button takes the currently selected page-summary rows and either creates a new named aggregation or appends them to an existing one (deduped). Modal picker. The modal also has an **Include aggregations as members** checklist, so an aggregation can contain other aggregations (`childAggIds`) as well as / instead of pages; the button is enabled with zero page rows when any aggregation exists, allowing member-only aggregations.
- A second entry point lives on the **AGG Summary table**: a leading *Select* column (per-row + select-all checkboxes) and its own **Create / Add Aggregation** button. Checking aggregation rows and clicking it opens the same modal with those rows pre-checked as members and zero pending page refs — the canonical way to build bundled / "meta" aggregations from existing groups. Reuses the existing member / cycle-guard logic; no new data model.
- **Recursive expansion**: `expandAggregation()` flattens an aggregation to an ordered, de-duplicated page list — member aggregations first (listed order, recursed), then own direct pages — with a cycle guard so a malformed tree terminates. The modal disables member choices that would form a cycle (self / existing member / would-loop); `deleteAggregation` detaches the deleted id from all parents.
- **AGG Summary table** mirrors the page-table column layout: *Name, Parent, Pages, Thumbnail (first expanded page), Notes, LLM Response, per-prompt columns, Delete*. The **Parent** column shows the containing-aggregation path(s) (`Root / Mid`, multiple joined by `; `), blank for top-level. **Pages** is the expanded count, tagged *(expanded)* for composites. Aggregation-level Notes are independent of page-level notes. Prompt columns reflect the *Summary table heading* set (read-only display).
- **AGG Detail table** appears when an aggregation is selected; it shows the expanded page list. Its columns are an independent set, driven by the *Aggregation detail heading* flag. For composite aggregations the rows are read-only (no drag-reorder, no per-row remove) since pages belong to member aggregations; leaf aggregations keep reorder/remove. Apply operates on the expanded list; Save-as-PDF opens the PDF builder seeded from it (or the checked subset).
...
- **Save as PDF** (experimental) opens a **PDF builder** modal seeded with the resolved page set — the checked agg-detail rows, or all expanded pages if none are checked. In the modal each page is a draggable card with a per-page column checklist (Notes / LLM Response / agg-detail prompt columns; only columns with content are offered, all checked by default) and a remove control; order and removals are scoped to that one export and do not mutate the aggregation. On save, `pdf-lib` copies each source page in the builder's order and, when columns are ticked for that page, inserts a generated **annotation page** directly after it (Helvetica, WinAnsi-sanitized, word-wrapped, paginating as needed; image-result columns embedded as PNG/JPEG). Source PDFs are cached by `fileId` so multi-page aggregations from one file only parse once. Unticking all columns reproduces the prior image-only export.
...
- Saved content: prompt library (all flags), file entries with stable `fileId`s, page-level notes / LLM response / per-prompt result cells, aggregations (name, notes, page order, page refs, `childAggIds`, `promptResults`), `selectedAggregationId`, and `nextId`.
